...
// 6-byte positional payload: temp | hum | dust ADC
...
void do_send(osjob_t* j) {
    ...
    // Compute dust density locally for Serial monitor only
    float voltage = adc * (5.0 / 1023.0);
    float dust    = 170.0 * voltage - 100.0;
    if (dust < 0) dust = 0;

    Serial.print(F("T:")); Serial.print(temp, 1);
    Serial.print(F("C H:")); Serial.print(hum, 1);
    Serial.print(F("% ADC:")); Serial.print(adc);
    Serial.print(F(" Dust:")); Serial.print(dust, 1); Serial.print(F("ug/m3"));
    Serial.print(F(" ["));
    for (int i = 0; i < 6; i++) {
...
void setup() {
    Serial.begin(9600);
    Serial.println(F("DHT22 + GP2Y1010AU0F Dust | ABP | Single-CH 868.1 MHz"));
