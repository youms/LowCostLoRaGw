...
echo "indicating eu1.cloud.thethings.network as LoRaWAN network server in /opt/ttn-gateway/packet_forwarder/lora_pkt_fwd/global_conf.json"
#sudo sed -i -- 's/server_address".*".*"/server_address": "eu1.cloud.thethings.network"/g' /home/pi/lora_gateway/global_conf.json
sudo sed -i -- 's/server_address".*".*"/server_address": "eu1.cloud.thethings.network"/g' /opt/ttn-gateway/packet_forwarder/lora_pkt_fwd/global_conf.json
