...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
        ...
        // Calculate gas concentration ratio (this is a simplified calculation)
        // For accurate readings, you'd need calibration with known gas concentrations
        float gasRatio = voltage / 5.0;
        
        Serial.print(F("MQ2 Raw Value: "));
        Serial.print(sensorValue);
        Serial.print(F(" | Voltage: "));
        Serial.print(voltage);
        Serial.print(F("V | Gas Ratio: "));
        Serial.println(gasRatio);
        
        // Prepare upstream data transmission
        // Pack sensor value (0-1023) into 2 bytes
        mydata[0] = highByte(sensorValue);
        mydata[1] = lowByte(sensorValue);
        
        // Prepare upstream data transmission at the next possible time
        LMIC_setTxData2(1, mydata, sizeof(mydata), 0); // port 1, 2 bytes, no ack
        
        Serial.print(F("Packet queued - MQ2 Value: "));
        Serial.print(sensorValue);
        Serial.println(F(" (Gas Detection)"));
        
        // Alert if gas level is high (customize threshold as needed)
        if (sensorValue > 300) {
...
void setup() {
    ...
    Serial.begin(115200);
    Serial.println(F("Starting MQ2 Gas Sensor with MCCI LMIC & TTN"));
    ...
    int initialReading = analogRead(MQ2_PIN);
