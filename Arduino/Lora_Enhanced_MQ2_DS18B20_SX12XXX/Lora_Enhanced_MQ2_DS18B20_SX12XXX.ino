/*
 *  Combined DS18B20 Temperature + MQ2 Multi-Gas Sensor with LoRa transmission
 *  Enhanced environmental monitoring station with low power, ACK, EEPROM features
 *  Using SX12XX library for LoRa communication
 *  
 *  Reads temperature from DS18B20 and multiple gas concentrations from MQ2
 *  Compatible with Congduc Pham's Low-Cost LoRa Gateway
 *
 *  Hardware connections:
 *  - DS18B20 data pin -> Arduino Pin 3
 *  - MQ2 sensor AO pin -> Arduino Pin A0
 *  - LoRa module as defined in DS18B20_MQ2_LoRa_SX12XXX.h
 */

#include <OneWire.h>
#include <DallasTemperature.h>
#include <SPI.h>
#include "Lora_Enhanced_MQ2_DS18B20_SX12XXX.h"
...
uint8_t node_addr = 14;  // Unique address for combined environmental sensor
//////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////
// CHANGE HERE THE TIME IN MINUTES BETWEEN 2 READING & TRANSMISSION
unsigned int idlePeriodInMin = 8;  // Balanced interval for environmental monitoring
///////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////
// LARGE MESSAGE BUFFER FOR COMBINED DATA
uint8_t message[200];  // Large buffer for temperature + multi-gas data
///////////////////////////////////////////////////////////////////
...
void setup()
{
  ...
  PRINT_CSTSTR("🌡️🔬 Combined Temperature + Multi-Gas Environmental Monitor (Modular)\n"); 
  PRINT_CSTSTR("================================================================\n");
  PRINT_CSTSTR("DS18B20 Temperature + MQ2 Gas Sensor with LoRa SX12XX\n");
  PRINT_CSTSTR("================================================================\n");
  ...
  PRINTLN_CSTSTR("🔥 Warming up MQ2 gas sensor...");
  ...
  PRINTLN_CSTSTR("For accurate gas readings, calibrate MQ2 in clean air");
  ...
  PRINTLN_CSTSTR("Environmental monitoring station ready");
  ...
  PRINTLN_CSTSTR("🚨 Environmental alerts enabled");
#endif

  // Initial MQ2 calibration reading
  delay(1000);
  int initial_adc = analogRead(MQ2_PIN);
  float initial_Rs = calculateResistance(initial_adc);
  
  PRINT_CSTSTR("Initial MQ2 resistance (R0 estimate): ");
...
void loop()
{
  ...
  if (millis() > nextTransmissionTime) {
    ...
    PRINTLN_CSTSTR("🔄 Reading environmental sensors...");
    
    // === READ DS18B20 TEMPERATURE SENSOR ===
    PRINT_CSTSTR("📊 Reading temperature... ");
    
    // Take multiple temperature readings for accuracy
    tempC = 0.0;
    for (int i=0; i<5; i++) {
    ...
    // === READ MQ2 GAS SENSOR ===
    PRINT_CSTSTR("💨 Reading gas concentrations... ");
    ...
    if (gasError) {
      PRINTLN_CSTSTR("❌ MQ2 Error");
    } else {
      PRINTLN_CSTSTR("✅");
    }
    
    // Set overall error flag
    sensorError = tempError && gasError;  // Both failed
    
    // Only transmit if we have at least one working sensor
    if (!sensorError) {
      // Gas calculations (only if gas sensor working)
      float lpg_ppm = 0, methane_ppm = 0, hydrogen_ppm = 0, co_ppm = 0, alcohol_ppm = 0;
      float rs_r0_ratio = 0;
      
      if (!gasError) {
        // Calculate Rs/R0 ratios (using average R0 for simplicity)
        float R0_avg = (R0_LPG + R0_METHANE + R0_CO) / 3.0;
        rs_r0_ratio = Rs / R0_avg;
        
        // Calculate gas concentrations (ppm)
        lpg_ppm = calculateLPG(rs_r0_ratio);
        methane_ppm = calculateMethane(rs_r0_ratio);
        hydrogen_ppm = calculateHydrogen(rs_r0_ratio);
        co_ppm = calculateCO(rs_r0_ratio);
        alcohol_ppm = calculateAlcohol(rs_r0_ratio);
      }
      
      // Display comprehensive readings
      PRINTLN_CSTSTR("=== ENVIRONMENTAL MONITORING REPORT ===");
      
      // Temperature section
      if (!tempError) {
        PRINT_CSTSTR("🌡️  Temperature: ");
        PRINT_VALUE("%.2f", tempC);
        PRINTLN_CSTSTR("°C");
      } else {
        PRINTLN_CSTSTR("🌡️  Temperature: SENSOR ERROR");
      }
      ...
      if (!gasError) {
        ...
        PRINT_CSTSTR("☠️  CO: "); PRINT_VALUE("%.1f", co_ppm); PRINTLN;
        PRINT_CSTSTR("🍷 Alcohol: "); PRINT_VALUE("%.1f", alcohol_ppm); PRINTLN;
      } else {
        PRINTLN_CSTSTR("💨 Gas Sensors: SENSOR ERROR");
      }
      
#ifdef ENVIRONMENTAL_ALERTS
      // Environmental alert analysis
      String alerts = getEnvironmentalAlert(tempError ? 20.0 : tempC, lpg_ppm, methane_ppm, co_ppm, alcohol_ppm);
      PRINT_CSTSTR("🚨 Alert Status: "); PRINT_STR("%s", alerts.c_str()); PRINTLN;
      
      // Critical warnings
      if (!gasError) {
        if (co_ppm > 50) {
            PRINTLN_CSTSTR("⚠️  WARNING: Carbon Monoxide detected!");
        }
        if (lpg_ppm > 1000 || methane_ppm > 1000) {
            PRINTLN_CSTSTR("⚠️  WARNING: Combustible gas detected!");
        }
      }
      if (!tempError) {
        if (tempC > 40) {
            PRINTLN_CSTSTR("⚠️  WARNING: High temperature detected!");
        } else if (tempC < 5) {
            PRINTLN_CSTSTR("⚠️  WARNING: Low temperature detected!");
        }
      }
#endif
      
      // Prepare comprehensive LoRa message
      uint8_t r_size;
      char temp_str[10], lpg_str[10], ch4_str[10], co_str[10], h2_str[10], alcohol_str[10];
      ...
      if (!gasError) {
        ...
        strcpy(co_str, "ERR");
        strcpy(h2_str, "ERR");
        strcpy(alcohol_str, "ERR");
      }
      
      // Create comprehensive environmental data message
      r_size = sprintf((char*)message, "\\!TC/%s/LPG/%s/CH4/%s/CO/%s/H2/%s/ALC/%s/ADC/%d", 
                      temp_str, lpg_str, ch4_str, co_str, h2_str, alcohol_str, 
                      gasError ? 0 : sensorValue);
      
      PRINT_CSTSTR("📡 Sending: ");
      PRINT_STR("%s", (char*)message);
      PRINTLN;
      
      PRINT_CSTSTR("📊 Payload size: ");
      ...
      if (LT.transmitAddressed(message, r_size, p_type, DEFAULT_DEST_ADDR, LT.readDevAddr(), 10000, MAX_DBM, WAIT_TX))
      {
        ...
        if (LT.readAckStatus()) {
          ...
          PRINTLN;
          PRINT_CSTSTR("📶 SNR of transmitted pkt: ");
      ...
      PRINTLN_CSTSTR("🌍 Environmental data transmitted successfully!");
      PRINTLN_CSTSTR("=========================================");
    }

#ifdef LOW_POWER
    // Enter sleep mode (whether we transmitted or had sensor error)
    PRINT_CSTSTR("😴 Entering power saving mode...\n");
...
#else
    PRINT_CSTSTR("⏰ Next transmission at: ");
