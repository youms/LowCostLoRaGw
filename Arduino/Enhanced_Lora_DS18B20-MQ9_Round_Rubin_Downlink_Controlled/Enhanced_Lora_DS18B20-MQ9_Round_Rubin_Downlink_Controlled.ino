...
typedef enum {
  ...
  STATE_MONITORING,   // SF9, 868.0MHz, carrier sense OFF
...
uint8_t monitoringConfigIndex = 6;  // Default: SF9-BW125-T50
...
uint32_t monitoringPeriod = 10;                            // 10 minutes monitoring
uint32_t calculateSleepDuration() {
  return (totalNodes - 1) * (monitoringPeriod + 1) * 60 * 1000; // +1 adds 2 minutes to sync with gateway
}
