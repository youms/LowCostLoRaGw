...
// Forward declarations for Scenario 2 state functions
extern void setMonitoringMode();
extern void setStandbyMode(); 
extern void setPostMonitorSleep();
    ...
    if (i < strlen(cmdstr)) {
      switch ((char)cmdstr[i]) {
        ...
        case 'L': 
          ...
          // Start monitoring mode /@M1#
          case 'M': 
            i++;
            cmdValue=getCmdValue(i, cmdstr);
            
            if (cmdValue == 1) {
              PRINTLN_CSTSTR("START_MONITOR command received");
              setMonitoringMode();
              configChanged = true;
            } else if (cmdValue == 0) {
              PRINTLN_CSTSTR("STOP_MONITOR command received");
              setPostMonitorSleep();
              configChanged = true;
            }
            break;

          // Force standby mode /@S1#  
          case 'S':
            i++;
            cmdValue=getCmdValue(i, cmdstr);
            
            if (cmdValue == 1) {
              PRINTLN_CSTSTR("STANDBY command received");
              setStandbyMode();
              configChanged = true;
            }
            break;
