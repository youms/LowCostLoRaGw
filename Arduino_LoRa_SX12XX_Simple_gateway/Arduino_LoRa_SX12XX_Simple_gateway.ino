...
// Set to true for ACK monitoring (inverted IQ)
// Set to false for data packet monitoring (normal IQ)
///////////////////////////////////////////////////////////////////
#define MONITOR_ACKS true
///////////////////////////////////////////////////////////////////
...
//run calibration after setting frequency
  ...
  // Configure IQ mode based on monitoring mode
#if MONITOR_ACKS
  LT.invertIQ(true);
  PRINTLN_CSTSTR("*** ACK MONITORING MODE (Inverted IQ) ***");
#else
  LT.invertIQ(false);
  PRINTLN_CSTSTR("*** DATA PACKET MONITORING MODE (Normal IQ) ***");
#endif
...
void loop()
{
  ...
  RXPacketL = LT.receive(RXBUFFER, RXBUFFER_SIZE, 10000, WAIT_RX); //wait for a packet to arrive with 60seconds (60000mS) timeout
  ...
  else
  {
    ...
    PRINTLN_CSTSTR("--> Monitoring all packets (promiscuous mode)");

    #if MONITOR_ACKS
        PRINTLN_CSTSTR("--> ACK packet captured");
    #else
        PRINTLN_CSTSTR("--> Data packet captured");
    #endif
